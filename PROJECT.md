...
a title, description, and a **banner image** (`og-image.png`). The banner is
generated by `tools/make-og.js` (a dependency-free Node script that draws a
1200×630 Flappy Bird scene + "FLAPPY BIRD" title and encodes a PNG via the
built-in `zlib`). Meta tags (`og:*`, `twitter:*`, `theme-color`) live in
`<head>` of `index.html`.
