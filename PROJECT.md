...
| `0cfdd63` | Add an **🏢 Office photo background** option: `BG_PHOTOS`/`bgImages` preload + cover-fit `bgImage()` (falls back to mountains if the image is missing, dims at night); new picker button + `BACKGROUNDS` entry. Photo lives at `backgrounds/office.jpg` |
